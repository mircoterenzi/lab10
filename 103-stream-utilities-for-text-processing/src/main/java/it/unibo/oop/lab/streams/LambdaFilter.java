...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        CHARNUM("Count the number of chars", string -> Integer.toString(string.length())),
        LINESNUM("Count the number of lines", string -> Long.toString(string.lines().count())),
        ORDERED("List all the words in alphabetical order", string -> List.of(string.split(" |\n")).stream()
                .sorted().collect(Collectors.joining(" "))),
        EACHWORDNUM("Write the count for each word", string -> List.of(string.split(" |\n")).stream()
                .collect(Collectors.groupingBy(String::valueOf, Collectors.counting())).entrySet().stream()
                .map(w -> w.getKey() + "->" + w.getValue()).collect(Collectors.joining(" ")));
